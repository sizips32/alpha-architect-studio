...
export const ExpressionEditor: React.FC<ExpressionEditorProps> = ({
  ...
  return (
    <div className="bg-gray-900/50 rounded-lg border border-gray-800 p-4 md:p-6 space-y-4">
      <div>
        <h2 className="text-lg font-bold text-gray-100">Alpha Expression</h2>
        <p className="text-sm text-gray-400">Define your alpha using mathematical and logical operators.</p>
        <textarea
          value={expression}
          onChange={(e) => setExpression(e.target.value)}
          placeholder="e.g., rank(close / delay(close, 1))"
      ...
      <div className="bg-gray-800/50 p-4 rounded-lg space-y-3">
        <label htmlFor="ai-idea" className="flex items-center space-x-2 text-sm font-medium text-gray-300">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
            ...
            >
            {isAiLoading ? <LoaderIcon className="w-5 h-5 animate-spin" /> : 'Generate'}
            </button>
        </div>
      </div>
    </div>
