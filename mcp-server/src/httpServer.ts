...
const ConfigSchema = z
  .object({
    ...
    delay: z.number().int().min(0).max(10).optional(), // Days of delay (0-10)
    lookbackDays: z.number().int().min(20).max(2000).optional(), // Historical data period (20-2000 days)
    maxStockWeight: z.number().min(0).max(10).optional(), // Max % weight per stock (0-10%)
    decay: z.number().int().min(0).max(30).optional(), // Decay period in days (0-30)
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/simulate_backtest') {
    try {
      ...
      const decayDays = config?.decay ?? 5; // Decay in days (default 5)
      const irAdj = (decayDays / 30) * 0.2 + (config?.delay ? -config.delay * 0.02 : 0);
      const maxW = (config?.maxStockWeight ?? 2) / 100; // Convert % to ratio (default 2%)
